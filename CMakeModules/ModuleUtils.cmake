...
macro(copy_shared_libs target libs source_dir)
    foreach(LIB ${libs})
        ...
        elseif(OMEGA_OS_OSX)
          postbuild_copy_files("${target}" "lib${LIB}.dylib" "${source_dir}" "${CMAKE_LIBRARY_OUTPUT_DIRECTORY}")
        else()
          postbuild_copy_files("${target}" "lib${LIB}.so" "${source_dir}" "${CMAKE_LIBRARY_OUTPUT_DIRECTORY}")
        endif()
    endforeach()
endmacro()